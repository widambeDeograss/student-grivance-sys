<template>
  <div v-if="alerts.length" class="fixed top-0 right-0 m-4 z-999999 ">
    <div v-for="(alert, index) in alerts" :key="index"
         :class="['flex', 'w-full', 'max-w-sm', 'mx-auto p-5', {
      ...
      <div :class="['mr-5', 'flex', 'h-9', 'w-9', 'items-center', 'justify-center', 'rounded-lg', {
        'bg-green-500': alert.type === 'success',
        'bg-red': alert.type === 'error',
        'bg-yellow-300': alert.type === 'warning'
      }]">
      ...
      <div class="w-full">
        <div class="flex justify-between items-center">
          <h5 class="mb-3 text-lg font-bold"
            :class="{
              ...
              'text-black': alert.type === 'warning'
            }">{{ alert.title }}</h5>


         <span class="font-semibold hover:scale-105 cursor-pointer"
        ...
        <p class="leading-relaxed"
           :class="
            alert.type === 'success'? 'text-white':alert.type === 'error'?
